...
class UnsupportedAssociationProxyTarget(Exception):
    def __init__(self, key):
        super().__init__(
            f"Association proxy `{key}` is expected to be of form association_proxy(relationship_name, other relationship name)"
        )
...
class InterfaceModelNotPolymorphic(Exception):
    def __init__(self, model):
        super().__init__(
            f"Model `{model}` is not polymorphic or is not the base model of its inheritance chain, and thus cannot be used as an interface."
        )
